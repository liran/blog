...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/bbr.sh -O bbr.sh; then
...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode="https://ssr-resource-proxy.wangshirufeng.workers.dev?url=http://doub.pw/qr/qr.php?text=${SSurl}"
...
ssr_link_qr(){
	...
	SSRQRcode="https://ssr-resource-proxy.wangshirufeng.workers.dev?url=http://doub.pw/qr/qr.php?text=${SSRurl}"
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://github.com/ToyoDAdoubiBackup/shadowsocksr/archive/manyuser.zip" -O manyuser.zip
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
Check_Libsodium_ver(){
	...
	Libsodiumr_ver=$(wget -qO- "https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://github.com/jedisct1/libsodium/tags"|grep "/jedisct1/libsodium/releases/tag/"|head -1|sed -r 's/.*tag\/(.+)\">.*/\1/')
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		wget  --no-check-certificate -N "https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_ver}-RELEASE/libsodium-${Libsodiumr_ver}.tar.gz" -O libsodium-${Libsodiumr_ver}.tar.gz
		echo -e "${Info} 解压..."
...
get_IP_address(){
	...
	#echo "user_IP_total=${user_IP_total}"
		...
		do
			...
			#echo "IP=${IP}"
			IP_address=`wget -qO- -t1 -T2 https://ssr-resource-proxy.wangshirufeng.workers.dev?url=http://freeapi.ipip.net/${IP}|sed 's/\"//g;s/,//g;s/\[//g;s/\]//g'`
...
Install_ServerSpeeder(){
	...
	cd /root
	#借用91yun.rog的开心版锐速
	wget -N --no-check-certificate https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/91yun/serverspeeder/master/serverspeeder.sh -O serverspeeder.sh
...
Install_LotServer(){
	...
	wget --no-check-certificate -qO /tmp/appex.sh "https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/0oVicero0/serverSpeeder_Install/master/appex.sh"
...
Uninstall_LotServer(){
	...
	[[ -z ${unyn} ]] && echo && echo "已取消..." && exit 1
	if [[ ${unyn} == [Yy] ]]; then
		wget --no-check-certificate -qO /tmp/appex.sh "https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/0oVicero0/serverSpeeder_Install/master/appex.sh" && bash /tmp/appex.sh 'uninstall'
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/ban_iptables.sh -O ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/ban_iptables.sh -O ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ssr" ]]; then
		rm -rf /etc/init.d/ssr
		Service_SSR
	fi
	wget -N --no-check-certificate "https://ssr-resource-proxy.wangshirufeng.workers.dev?url=https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/ssr.sh" -O ssr.sh && chmod +x ssr.sh
